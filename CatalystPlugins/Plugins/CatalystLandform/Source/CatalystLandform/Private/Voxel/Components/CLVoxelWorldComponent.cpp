...
int32 UCLVoxelWorldComponent::GetVoxelSize() const
{
	...
	{
		return FMath::Max(1, World->VoxelSize); // cm
	}
	return 1;
}
...
int32 UCLVoxelWorldComponent::GetResolution() const
{
	...
	const int32 VoxelSizeCm = GetVoxelSize();
	if (HeightM <= 0.0 || VoxelSizeCm <= 0)
	{
		return 1;
	}
	const double VoxelSizeM = double(VoxelSizeCm) / 100.0;
	...
	const bool bNormalize) const
{
	...
	// Miss -> write (still const function: map is mutable & guarded by RWLock)
...
{
	Super::OnRegister();
	// Optional: bind to world invalidation if desired later
	// if (AVoxelWorld* World = const_cast<AVoxelWorld*>(GetVoxelWorldOwner()))
	// {
	//     World->OnInvalidated(FSimpleDelegate::CreateUObject(this, &UCLVoxelWorldComponent::InvalidateCaches));
	// }
}
